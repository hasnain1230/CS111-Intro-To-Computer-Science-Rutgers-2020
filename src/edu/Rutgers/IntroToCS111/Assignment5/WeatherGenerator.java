...
public class WeatherGenerator {
    ...
    public static int[] oneMonthGenerator(double longitute, double latitude, 
        ...
        int[] oneMonthForecast = new int[numberOfDaysInMonth[month - 2]];
        double randomProbability = StdRandom.uniform(0.0, 1.0);
        double probabilityDryWet = Double.MIN_VALUE, probabilityWetWet = Double.MIN_VALUE;

        if (drywet.length == wetwet.length && drywet[0].length == wetwet[0].length) {
            for (int i = 0; (i < drywet.length && i < wetwet.length); i++) {
                if (drywet[i][0] == longitute && wetwet[i][0] == longitute &&
                        drywet[i][1] == latitude && wetwet[i][1] == latitude) {
                        probabilityDryWet = drywet[i][month];
                        probabilityWetWet = wetwet[i][month];
                        break;
                }
            }
        }
        for (int x = 0; x < numberOfDaysInMonth[month - 2]; x++) {
            if (x == 0) {
                if (randomProbability < .5) oneMonthForecast[x] = WET;
                else oneMonthForecast[x] = DRY;
            } else {
                if (oneMonthForecast[x - 1] == WET) {
                    randomProbability = StdRandom.uniform(0.0, 1.0);

                    if (randomProbability <= probabilityWetWet) oneMonthForecast[x] = WET;
                    else oneMonthForecast[x] = DRY;
                } else if (oneMonthForecast[x - 1] == DRY) {
                    randomProbability = StdRandom.uniform(0.0, 1.0); ;

                    if (randomProbability <= probabilityDryWet) oneMonthForecast[x] = WET;
                    else oneMonthForecast[x] = DRY;
                }
            }
        }
    ...
    public static int numberOfWetDryDays (int[] forecast, int mode) {
        ...
        for (int dryOrWet : forecast) {
            if (dryOrWet == mode) numOfMmode++;
        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
       int currentStreak = 0, biggestStreak = 0, previousIndex = 0;

        for (int index = 0; index < forecast.length; index++) {
            if (currentStreak == 0 && forecast[index] == mode) {
                previousIndex = index;
                currentStreak++;
            } else if (forecast[index] == mode && index - previousIndex == 1) {
                previousIndex = index;
                currentStreak++;
            } else {
                currentStreak = 0;
            ...
            biggestStreak = Math.max(currentStreak, biggestStreak);
    ...
    double[][] wetwet, int numberOfLocations) {
        ...
        StdIn.setFile("src/edu/Rutgers/IntroToCS111/Assignment5/drywet.txt");
        ...
        // The first line on the file has number of locations (lines)
        StdIn.setFile("src/edu/Rutgers/IntroToCS111/Assignment5/wetwet.txt");
